...
- `node tools/npm/run-script.mjs priority:artifact:download -- --repo <owner/repo> --run-id <id> --artifact <name>`  
  ...
  step-summary lines. The helper now also records `destinationRootPolicy` so downstream tooling can prove whether the
...
- `release:finalize` also blocks when pre-tag compare evidence is incomplete:
  - requires latest successful `validate.yml` and `fixture-drift.yml` runs for the release branch, with artifacts attached.
...
- Run `node tools/npm/run-script.mjs priority:queue:supervisor -- --dry-run` to preview queue ordering and
  ...
  so the standing throughput surface can surface
  `pass|breach|overridden|platform-stale|skip|error` without rerouting the
  security workflow.
